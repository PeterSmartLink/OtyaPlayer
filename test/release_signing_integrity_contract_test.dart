...
void main() {
  test('direct-install release is manual and uses Android APK verification', () {
    final source = File('.github/workflows/release-apk.yml').readAsStringSync();
  ...
  test('production release tag must exactly match a semantic pubspec build', () {
    ...
    expect(source, contains(r'^[0-9]+\.[0-9]+\.[0-9]+\+[1-9][0-9]*$'));
    expect(source, contains('expected Otya <version>+<positive build>'));
  });

  test('production release verifies both APK signers before publication', () {
    final source = File('.github/workflows/release.yml').readAsStringSync();
    expect(source, contains(r'"$APKSIGNER" verify --verbose --print-certs "$ARM64"'));
    expect(source, contains(r'"$APKSIGNER" verify --verbose --print-certs "$ARM32"'));
