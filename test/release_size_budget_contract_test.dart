...
void main() {
  test('public split APKs stay within the Otya release size budget', () {
    ...
    expect(publisher, contains('[ "$SIZE" -le "$MAX_APK_BYTES" ]'));
    expect(
      publisher,
      contains('Otya split APKs must stay at or below $MAX_APK_BYTES bytes'),
    );
    expect(
      publisher.indexOf('[ "$SIZE" -le "$MAX_APK_BYTES" ]'),
      lessThan(publisher.indexOf('upload_and_verify "$ARM64_APK"')),
    );
  });
}
